...
export const SignUp = () => {
  ...
  return (
    <div className='flex flex-col justify-center items-center h-screen gap-y-[0.5rem]'>
        <form onSubmit={handleSubmit(onSubmit)} className='border-[1px] border-[#DBDBDB] w-[24.875rem] p-[2.4rem] pt-[1rem] flex flex-col gap-y-[2.5rem] rounded-[3px]'>
            ...
            <div className='flex flex-col gap-y-[6px]'>
                {
                    SignupInput?.map((data) => (
                        <div key={data?.Id} className='flex items-center relative border-[#DBDBDB] border-[1px] bg-[#FAFAFA] pr-[0.5rem]'>
                            <input type={data?.Type} name={data?.Name} required {...register(`${data?.Name}`, { required: true })}
                            className='bg-[#FAFAFA] leading-[2.25rem] rounded-sm outline-none px-[0.5rem] w-full h-[3.4rem] pt-[0.6rem]'/>
                            <span className="floating-label text-[0.75rem] text-[#737373]">{data?.Placeholder}</span>
            ...
            <div className='text-[0.75rem] -mt-[1.5rem] flex flex-col gap-y-[1rem]'>
                <p className='text-[#737373] font-normal text-center'>
                    People who use our service may have uploaded your contact information to Instagram. <span className='text-[#385898] cursor-pointer'>Learn more</span>
                </p>
                <p className='text-[#737373] font-normal text-center'>
                    By signing up, you agree to our <span className='text-[#385898] cursor-pointer'>
                        Terms, Privacy Policy </span> and <span className='text-[#385898] cursor-pointer'>Cookies Policy.</span>
                </p>
            </div>
